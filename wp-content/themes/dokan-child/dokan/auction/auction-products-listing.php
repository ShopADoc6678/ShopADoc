...
<?php */?>
                    <?php if ( current_user_can( 'dokan_add_auction_product' ) ): ?>
                        <span class="dokan-add-product-link">
                            <a href="<?php echo dokan_get_navigation_url( 'new-auction-product' ); ?>" class="dokan-btn dokan-btn-theme dokan-right"><i class="fa fa-briefcase">&nbsp;</i> <?php _e( 'Add New Auction Product', 'dokan-auction' ); ?></a>
                        </span>
                    <?php endif ?>
                </div>
                ...
                <div class="dokan-alert dokan-alert-danger">
                    <a class="dokan-close" data-dismiss="alert">&times;</a>
                        <strong><?php _e( 'Error!', 'dokan-auction' ); ?></strong> You can not relist this auction.<br>
                </div>
            <?php }elseif (isset($_GET['action']) && $_GET['action'] =='update_success' && 1==2) { ?>
            	<div class="dokan-alert dokan-alert-success">
                    <a class="dokan-close" data-dismiss="alert">&times;</a>
                        <strong><?php _e( 'Success!', 'dokan-auction' ); ?></strong> Ask fee has been successfully updated.<br>
                </div>
            <?php }elseif (isset($_GET['action']) && $_GET['action'] =='update_error') {?>
            	<div class="dokan-alert dokan-alert-danger">
                    <a class="dokan-close" data-dismiss="alert">&times;</a>
                        <strong><?php _e( 'Error!', 'dokan-auction' ); ?></strong> Ask fee should be greater than or equal to current price.<br>
                </div>
            <?php }?>
            <style type="text/css">
			.table > thead > tr > th {
            ...
            <div class="nano">
             <div class="my-listing-custom table-wrap">
                        ...
                        if ( $product_query->have_posts() ) {
                            while ($product_query->have_posts()) {
                                ...
                                <tr<?php echo $tr_class; ?>>
                                    ...
                                    <td width="30%" class="service_title">
                                        ...
                                        <div class="row-actions">
											...
											?>
                                            <span class="relist"><a href="<?php echo $Relist_url; ?>"><?php _e( 'Relist', 'dokan-auction' ); ?></a> | </span>
                                            <?php endif ?>
                                            <?php if ( current_user_can( 'dokan_edit_auction_product' ) && 1==2): ?>
                                                <span class="edit"><a href="<?php echo $edit_url; ?>"><?php _e( 'Edit', 'dokan-auction' ); ?></a> | </span>
                                            <?php endif ?>

                                            <?php if ( current_user_can( 'dokan_delete_auction_product' ) && 1==2): ?>
                                                <span class="delete"><a onclick="return confirm('Are you sure?');" href="<?php echo wp_nonce_url( add_query_arg( array( 'action' => 'dokan-delete-auction-product', 'product_id' => $post->ID ), dokan_get_navigation_url('auction') ), 'dokan-delete-auction-product' ); ?>"><?php _e( 'Delete Permanently', 'dokan-auction' ); ?></a> | </span>
                                            <?php endif ?>
									...
									?>
                                    <td class="post-status center status_col" <?php echo $width;?>>
                                        <label class="dokan-label <?php //echo $post->post_status.$class; ?>"><?php echo $status;//dokan_get_post_status( $post->post_status ); ?></label>
                                    </td>
                                    <td width="20%" class="post-status center date_col" valign="top">
                                       <label class="dokan-label"><?php echo trim(date_i18n('l m/d/Y',strtotime($_auction_dates_from)));?></label>
                                    </td>
