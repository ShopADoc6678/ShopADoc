...
class Payment {
    ...
    public function redirect_order() {
        ...
        if ( 'FAILED' === $transaction->Status ) {
            wc_add_notice( '<span class="cancelmessagealone">' . sprintf( __( '%s', 'dokan' ), $transaction->ResultMessage ) . '</span>', 'error' );
            $order->update_status( 'failed', __( sprintf( __( '%s', 'dokan' ), $transaction->ResultMessage ), 'dokan' ) );
