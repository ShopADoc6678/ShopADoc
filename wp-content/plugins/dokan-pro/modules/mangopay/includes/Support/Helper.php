...
class Helper {
    ...
    private static $restricted_residences = array(
        'AF', 'AL', 'BS', 'BB', 'BW', 'BF', 'KH', 'KY', 'ET', 'GH', 'HT', 'IR', 'IQ', 'JM', 'MM', 'NI', 'KP', 'UG', 'PK', 'PH','SN', 'SS', 'SY', 'TT', 'VU', 'YE', 'ZW',
    );
    ...
    public static function get_locale() {
        ...
        if ( in_array( $locale_minor, self::$supported_locales ) ) {
   ...
   public static function is_vendor_subscription_product( $product ) {
    ...
    public static function get_years_dropdown( $limit = 10 ) {
        ...
        for ( $year = $this_year; $year <= $limit; ++$year ) {
            $years[ substr( $year, 2, 2 ) ] = sprintf( __( '%s', 'dokan' ), $year );
        }
    ...
    public static function get_months_dropdown() {
        ...
        for ( $i = 1; $i <= 12; ++$i ) {
            $months[ $i ] = sprintf( __( '%s', 'dokan' ), date( 'F', mktime( 0, 0, 0, $i, 1 ) ) );
        }
