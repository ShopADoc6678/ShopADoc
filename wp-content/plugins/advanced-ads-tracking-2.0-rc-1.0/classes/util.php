...
final class Advanced_Ads_Tracking_Util {
	...
	/**
	 * @var array Array of bots that should not get ads displayed but not trigger ad impressions/clicks.
	 *            Gets added to bots list from main plugin.
	 */
	protected $bots = array( 'mediapartners', 'ecosia', 'datanyze', 'AspiegelBot', 'Googlebot' );
	...
	public function collect_blog_data() {
		...
		if ( !isset( $this->blog_data['ajaxurls'][$bid] ) ) {
			$handler = content_url( 'ajax-handler.php' );
			if ( ! file_exists( WP_CONTENT_DIR . '/ajax-handler.php' ) || ( defined( 'ADVANCED_ADS_TRACKING_LEGACY_AJAX' ) && ADVANCED_ADS_TRACKING_LEGACY_AJAX ) ) {
		...
		if ( !isset( $this->blog_data['methods'][$bid] ) ) {
			$this->blog_data['methods'][$bid] = $this->plugin->get_tracking_method();
		}
	 ...
	 *
	 * @param array    $args       Arguments for tracking call.
	 * @param int|null $start_time Start time of tracking request, used for metrics. 
	 *
	 * @since 1.0.0
	 */
	public function track_impression( $args = array(), $start_time = null ) {
		$ad_id = isset( $args['ad_id'] ) ? (int) $args['ad_id'] : 0;
		if ( ! $this->is_tracking_allowed( $ad_id ) ) {
			return;
		}

		$this->persist( $ad_id, $this->impressions_table, $start_time );
	 ...
	 * @since 1.1.0
	 */
	public function track_click( $args = array(), $start_time = null ) {
		$ad_id = isset( $args['ad_id'] ) ? (int) $args['ad_id'] : 0;
		if ( ! $this->is_tracking_allowed( $ad_id ) ) {
			return;
		}
		$this->persist( $ad_id, $this->clicks_table, $start_time );
	}
	 ...
	 * @return bool
	 */
	private function is_tracking_allowed( $ad_id ) {
	 ...
	 */
	public function add_bots_triggering_ajax( $bots ) {
		return array_merge( $bots, $this->bots );
	}

	protected function persist($id, $table, $start_time = null) {
	    global $wpdb;
		...
		if ( Advanced_Ads_Tracking_Debugger::debugging_enabled($id) ) {
			Advanced_Ads_Tracking_Debugger::log( $id, $table, ! is_null( $start_time ) ? round( ( microtime( true ) - $start_time ) * 1000 ) : null );
		}
