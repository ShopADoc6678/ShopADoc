...
class Advanced_Ads_Tracking_Plugin {
	 ...
	 * Default is `frontend` (AJAX) - If it's not defined, set the option.
	 *
	 * @return string
	 */
	public function get_tracking_method() {
		$options       = $this->options();
		$valid_methods = array( 'frontend', 'ga', 'shutdown', 'onrequest' );

		if ( array_key_exists( 'method', $options ) && is_string( $options['method'] ) && ! empty( $options['method'] ) && in_array( $options['method'], $valid_methods, true ) ) {
