jQuery(document).ready(function($){
	...
	$(document).on('click', '.swf_add_to_favourites', function(e){
		...
		var html = "<button class='swf_remove_from_favourites' data-product_id='"+prod_id+"'><i class='fa fa-heart'>&nbsp;</i></button>&nbsp;<span class='sav-fav-text'>Saved</span>";
		$this_button.closest('.swf_container').html(html);
		$.post(swfAjax.ajaxurl, data, function(msg){
			//var html = "<span class='swf_message'>"+msg+"</span><button class='swf_remove_from_favourites' data-product_id='"+prod_id+"'><i class='fa fa-heart'>&nbsp;</i></button>";
