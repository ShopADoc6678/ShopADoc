...
function updateAuction(){
	...
	jQuery.ajax({
		...
		dataType: 'json',
		data : {action: "get_update_price_for_auctions",mode:'checkBidUpdate', "product_id" : product_id,'current_bid':current_bid,"last_activity" : SA_last_activity,'current_user_ID':current_user_ID,'price':price,'distance':distance},
		success: function(response) {
			if(response != null ) {
				//bid_value==value==max
				...
				if(response.add_to_cart_text!='Auction finished'){
					if(response.response_mode=='update_distance'){
						...
						jQuery(".next_bid").html('<span class="woocommerce-Price-currencySymbol">$</span><span class="underline_amt">'+response.price+"</span>");
