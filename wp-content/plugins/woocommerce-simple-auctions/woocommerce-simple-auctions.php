	...
	if ( ! class_exists( 'WooCommerce_simple_auction' ) ) {
		...
		class WooCommerce_simple_auction {
			...
			public function ajax_place_auction_bid( $url = false ) {
				...
				$bid               = apply_filters( 'woocommerce_place_bid_bid', abs( round( str_replace( ',', '', $_REQUEST['bid_value'] ), wc_get_price_decimals_mujahid() ) ) );
...
Use upper-case characters for mandatory periods, or the corresponding lower-case characters for optional periods, i.e. only display if non-zero. Once one optional period is shown, all the ones after that are also shown.",
					...
					array(
						...
						'desc'    => __( 'Maximum value for single bid. Default value is ', 'wc_simple_auctions' ) . wc_price_mujahid( '999999999999999999.99' ),
...
?>
												...
												<p><?php _e( 'Product sold for buy now price', 'wc_simple_auctions' ); ?>: <span><?php echo wc_price_mujahid( $product_data->get_regular_price() ); ?></span></p>
					...
					?>
						...
						<p><?php _e( 'Highest bid was', 'wc_simple_auctions' ); ?>: <span class="higestbid" ><?php echo wc_price_mujahid( $product_data->get_curent_bid() ); ?></span></p>
					...
					if ( $product_data->is_closed() ) {
						...
						if ( isset( $_POST['ret'] ) && $_POST['ret'] != '0' ) {
							...
							if ( $product_data->get_auction_current_bider() ) {
								...
								printf( __( 'Winning bid is %1$s by %2$s.', 'wc_simple_auctions' ), wc_price_mujahid( $product_data->get_curent_bid() ), apply_filters( 'woocommerce_simple_auctions_displayname', get_userdata( $product_data->get_auction_current_bider() )->display_name, $product_data ) );
				...
				if ( $_POST['postid'] && $_POST['logid'] ) {
					...
					if ( ! is_null( $log ) ) {
						...
						if ( $product ) {
								...
								$return['auction_current_bid'] = wc_price_mujahid( $product->get_curent_bid() );
