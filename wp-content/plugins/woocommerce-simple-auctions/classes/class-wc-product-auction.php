...
class WC_Product_Auction extends WC_Product {
	...
	function get_curent_bid() {
			...
			if ($this->get_auction_current_bid()){
				//mujahid code
				return apply_filters('woocommerce_simple_auctions_get_current_bid' ,custom_number_format($this->get_auction_current_bid()),$this);

			}
			//mujahid code
			return apply_filters('woocommerce_simple_auctions_get_current_bid' ,custom_number_format($this->get_auction_start_price()), $this);
	...
	function is_finished() {
		...
		if (!empty($this->get_auction_dates_to()) ){
			...
			$date2 = new DateTime(current_time('mysql'));
			//Mujahid Code
			$_auction_dates_extend = get_post_meta($this->get_id(), '_auction_dates_extend',TRUE);
	       $_auction_extend_counter = get_post_meta($this->get_id(), '_auction_extend_counter',TRUE);
		   if($_auction_dates_extend=='yes' && $_auction_extend_counter=='yes'){
					global $today_date_time;
					$auctionend = new DateTime($this->get_auction_dates_to());
					$auctionendformat = $auctionend->format('Y-m-d H:i:s');
					$time = current_time( 'timestamp' );
					$date2 = date('Y-m-d H:i:s',strtotime($today_date_time));
					$auctionend->add(new DateInterval('PT300S'));
					$date1 = $auctionend->format('Y-m-d H:i:s') ;
			}
			//Mujahid Code End
			if( $date1 < $date2){
	...
	function auction_history_last($id) {
		...
		if($history_value){
			...
			$data = "<tr>";
	            ...
	            $data .= "<td class='bid'>".wc_price_mujahid($history_value->bid)."</td>";
	...
	public function get_price_html( $price = '' ) {
		...
		if ($this->is_closed() && $this->is_started() ){
			...
			if ($this->get_auction_closed() == '3'){
				...
				$price = __('<span class="sold-for auction">Sold for</span>: ','wc_simple_auctions').wc_price_mujahid($this->get_price());
				...
				if ($this->get_auction_current_bid()){
					...
					if ( $this->is_reserve_met() == FALSE){
						...
						$_auction_start_price = get_post_meta( $id, '_auction_start_price',TRUE);
						if(is_admin()){
							$price = wc_price_ask_mujahid($_auction_start_price).' / ';
							if($_auction_start_price <=1){
								$price .= 'None';
							}else{
								$price .=wc_price_mujahid($this->get_auction_current_bid());
							}
						}else{
							if($_auction_start_price <=1){
								$_auction_current_bid = get_post_meta( $id, '_auction_current_bid', true );
								if($_auction_current_bid==.99 ){
									$price = __('<span class="winned-for auction">Current Bid:</span> ','wc_simple_auctions').'<span class="woocommerce-Price-amount amount">$<span class="underline_amt underline">0</span></span>';
								}else{
									$price = __('<span class="winned-for auction">Current Bid:</span> ','wc_simple_auctions').'None';
								}
							}else{
								$price = __('<span class="winned-for auction">Current Bid:</span> ','wc_simple_auctions').wc_price_mujahid($this->get_auction_current_bid());
							}
						}
			
						//$price = '<span class="auction-price starting-bid" data-auction-id="'.$id.'" data-bid="" data-status="running"><span class="starting auction red">Next Bid:&nbsp;</span><span class="woocommerce-Price-amount amount">$0</span></span>';
			...
			$price = '<span class="auction-price starting-bid" data-auction-id="'.$id.'" data-bid="'.$this->get_auction_current_bid().'" data-status="future">'.__('<span class="starting auction">Starting bid:</span> ','wc_simple_auctions').wc_price_mujahid($this->get_curent_bid()).'</span>';
			$post   = get_post( $id );
			$user_id = get_current_user_id();
			$user = get_userdata( $user_id );
			if ($user_id == $post->post_author && 1==2){
			...
			if($this->get_auction_sealed() == 'yes'){
				...
				if (!$this->get_auction_current_bid()){
					...
					$price = '<span class="auction-price starting-bid" data-auction-id="'.$id.'" data-bid="'.$this->get_auction_current_bid().'" data-status="running">'.__('<span class="current auction">Starting bid:</span> ','wc_simple_auctions').wc_price_mujahid($this->get_curent_bid()).'</span>';
					if ($user_id == $post->post_author && 1==2){
						$price .='<form class="price_form" action="'.get_permalink().'" method="post" onsubmit="playAudio(\''.home_url('wp-content/uploads/sounds/Money%20(MP3).mp3').'\');"><input type="hidden" value="'.$this->get_curent_bid().'" id="_new_price" name="_new_price" /><span><img src="'.home_url('wp-content/themes/dokan-child/Up.png').'" onclick="updatePrice(\'up\');" alt="up" class="up_img" border="0" style="width:40px;"/><img id="down_price_btn" src="'.home_url('wp-content/themes/dokan-child/Down.png').'" onclick="updatePrice(\'down\');" alt="down" class="down_img" border="0" style="width:40px;"/></span><span><input type="image" src="'.home_url('wp-content/themes/dokan-child/Update.png').'" name="update_distance" id="update_price" style="width:100px;padding:0;border:0;background:transparent;vertical-align:middle;"/></span>
						<input type="hidden" name="mode" value="update_price" />
				</form>';
					...
					$price = '<span class="auction-price current-bid" data-auction-id="'.$id.'" data-bid="'.$this->get_auction_current_bid().'" data-status="running">'.__('<span class="current auction">Current Bid:</span> ','wc_simple_auctions').wc_price_mujahid($this->get_curent_bid()).'</span>';
	...
	public function bid_value() {
		...
		if ( ! $this->get_auction_current_bid() ) {
			...
			if($this->get_auction_type() == 'reverse' ){
				...
				//mujahid Code
				return apply_filters('woocommerce_simple_auctions_bid_value' ,custom_number_format( wc_format_decimal($this->get_curent_bid()) - wc_format_decimal($auction_bid_increment),wc_get_price_decimals_mujahid()), $this);
    ...
    public function get_auction_bid_increment( $context = 'view' ) {
		...
		if ($bid_amount > 0){
			...
			if($_auction_bid_increment < 1){
				$_auction_bid_increment = 1;
			}
		}else{
