...
class WC_Bid {
	...
	public function placebid($product_id, $bid) {
		...
		//echo $bid;die;
		//mujahid code
		$this->bid = apply_filters( 'woocommerce_simple_auctions_place_bid_value', $bid , $product_id) ;
        ...
        if ($current_user->ID == $product_data->get_auction_current_bider() && get_option( 'simple_auctions_curent_bidder_can_bid') === 'yes') {
        	 ...
        	 if ($product_data->get_auction_proxy() && $product_data->is_reserve_met() === TRUE ) {
        	 	...
        	 	if ($auction_type == 'normal') {
	        	 	...
	        	 	if (  $this->bid <= $product_data->get_auction_max_bid()  && get_option( 'simple_auctions_smaller_max_bid', 'no' ) === 'no') {
	        	 	...
	        	 	if( $this->bid <= str_replace(",","",$product_data->bid_value())){
		...
		if ($auction_type == 'normal') {
			...
			if ( apply_filters( 'woocommerce_simple_auctions_minimal_bid_value', str_replace(",","",$product_data->bid_value()), $product_data, $this->bid )<= ($this->bid )) {
					...
					if ($this->bid >$product_data->get_auction_max_bid()) {
						...
						if ($product_data->get_auction_reserved_price() && $product_data->is_reserve_met() === FALSE) {
							...
							if ($product_data->get_auction_max_bid()){
								...
								$curent_bid = ($this->bid < str_replace(",","",$product_data->bid_value())) ? $this->bid : str_replace(",","",$product_data->bid_value());		
			...
			if (apply_filters( 'woocommerce_simple_auctions_minimal_bid_value', str_replace(",","",$product_data->bid_value()), $product_data, $this->bid ) >= $bid || apply_filters( 'woocommerce_simple_auctions_minimal_bid_value', str_replace(",","",$product_data->bid_value()), $product_data, $this->bid ) == .99 ) {
				...
				if ($product_data->get_auction_proxy() ) {
					...
					if (  $this->bid < $product_data->get_auction_max_bid() OR  !$product_data->get_auction_max_bid() || apply_filters( 'woocommerce_simple_auctions_minimal_bid_value', str_replace(",","",$product_data->bid_value()), $product_data, $this->bid ) == .99) {
						...
						if($product_data->get_auction_max_bid() <  $product_data->get_auction_current_bid() || apply_filters( 'woocommerce_simple_auctions_minimal_bid_value', str_replace(",","",$product_data->bid_value()), $product_data, $this->bid ) == .99){
							if($_REQUEST['bid_value'] == .99){
								$this -> log_bid($product_id,.99, get_userdata( $current_user->ID), 1);
							}else{
								$this -> log_bid($product_id, $product_data->get_auction_max_bid(), get_userdata($product_data->get_auction_max_current_bider()), 1);
							}
						...
						$outbiddeduser = $product_data->get_auction_current_bider();
						if( $_REQUEST['bid_value'] == .99){
							update_post_meta($product_id, '_auction_max_bid', .99);
							update_post_meta($product_id, '_auction_max_current_bider', $current_user->ID);
							update_post_meta($product_id, '_auction_current_bid', .99);
						}else{
							update_post_meta($product_id, '_auction_max_bid', $this->bid);
...
die;
	...
	public function placebid_old($product_id, $bid) {
		...
		$this->bid = apply_filters( 'woocommerce_simple_auctions_place_bid_value', wc_format_decimal($bid) , $product_id) ;
		...
		if ($auction_type == 'normal') {
			...
			if (apply_filters( 'woocommerce_simple_auctions_minimal_bid_value', str_replace(",","",$product_data->bid_value()), $product_data, $this->bid ) >= $bid) {
				...
				if ($product_data->get_auction_proxy() ) {
					...
					if (  $this->bid < $product_data->get_auction_max_bid() OR  !$product_data->get_auction_max_bid()) {
	...
	function auction_sealed_placebid($product_data, $bid){
		...
		if ($auction_type == 'normal') {
			...
			if (  $this->bid > $product_data -> get_curent_bid()) {
