...
jQuery.fn.addSortWidget = function(options){
	var defaults = {
		img_asc: "/wp-content/plugins/WP_Sale_Graph/img/asc_sort.gif",	
        img_desc: "/wp-content/plugins/WP_Sale_Graph/img/desc_sort.gif",	
        ...
        jQuery("<img>")                              // create image that allows you to sort by specific column 
            ...
            .on('click', function(){
                ...
                rows.sort(function(a, b){               
					...
					window.console(jQuery("td:eq(" + index + ")", a).text());
                    var m = jQuery("td:eq(" + index + ")", a).text().replace(/,/g, '').replace(/$/g, ''); // get column you needed by using index of th element (closure)
                    var n =jQuery("td:eq(" + index + ")", b).text().replace(/,/g, '').replace(/$/g, '');
