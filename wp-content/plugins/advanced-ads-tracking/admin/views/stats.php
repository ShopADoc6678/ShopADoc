...
<div class="wrap">
	<h1><?php _e( 'Statistics', 'advanced-ads-tracking' ); ?></h1>
		...
		<table id="period-table">
			...
			<th>
				...
				if ( current_user_can( advanced_ads_tracking_db_cap() ) ) :
					?>
					<strong><?php _e( 'Data source:', 'advanced-ads-tracking' ); ?></strong><?php endif; ?></th>
			...
			<tr>
				<td>
					<fieldset class="load-from-db-fields">
						...
						<input type="text" name="advads-stats[from]" class="advads-stats-from<?php echo $period !== 'custom' ? ' hidden' : ''; ?>" value="<?php echo $from; ?>" autocomplete="off" size="10" maxlength="10" placeholder="<?php _e( 'from', 'advanced-ads-tracking' ); ?>"/>
						<input type="text" name="advads-stats[to]" class="advads-stats-to<?php echo $period !== 'custom' ? ' hidden' : ''; ?>" value="<?php echo $to; ?>" autocomplete="off" size="10" maxlength="10" placeholder="<?php _e( 'to', 'advanced-ads-tracking' ); ?>"/>
					...
					<label>
						<select name="advads-stats[groupby]">
							<?php foreach ( $groupbys as $_groupby_key => $_groupby ) : ?>
								<option value="<?php echo $_groupby_key; ?>" <?php selected( $_groupby_key, $groupby ); ?>><?php echo $_groupby[1]; ?></option>
							<?php endforeach; ?>
						</select>
