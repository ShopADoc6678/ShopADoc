...
(function ( $ ) {
    ...
    $('form.checkout, form#add_payment_method')
		...
		.on( 'change', '#wc-yith-stripe-cc-form .wc-credit-card-form-card-zip', function(){ card_zip = $(this).val(); })
		...
		.on( 'change', '#yith-stripe-cc-form .wc-credit-card-form-card-zip', function(){ card_zip = $(this).val(); });
        ...
        if ( $form.is('.add-card') || $( 'input#payment_method_yith-stripe' ).is( ':checked' ) && ( ! $( 'input[name="wc-yith-stripe-payment-token"]').length || $( 'input[name="wc-yith-stripe-payment-token"]:checked').val() == 'new' ) ) {
            ...
            if ( 0 === $( 'input.stripe-token' ).size() ) {
                ...
                var name_input   = $( '.wc-credit-card-form-card-name'),
                    ...
                    cvc          = cvc_input.val(),
					zip_input    = $( '.wc-credit-card-form-card-zip' ),
                    zip          = zip_input.val(),
                ...
                if ( zip == '' || (/(^\d{5}$)|(^\d{5}-\d{4}$)/).test(zip)===false) {
                    error = true;
                    fields.push( 'card.zip' );
                    zip_input.parents( 'p.form-row' ).addClass( 'error' );
                }
...
*/
                ...
                // go to payment
                    ...
                    $(document).trigger( 'yith_wcstripe_before_create_token', data );
